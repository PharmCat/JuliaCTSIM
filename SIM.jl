...
function a(BIN, Z, m1, sd1, m2, sd2, delta) #функция симуляции, обертка, для оптимального кода
    ...
    for i = 1:100000   #цикл из 100000 симуляций
        ...
        #Тест двухсторонний
        if 2*cdf(TDist(n1+n2-2), (mean(d1) - mean(d2) + delta)/(sqrt(((n1-1)*var(d1)+(n2-1)*var(d2))/(n1+n2-2)*(1/n1+1/n2)))) < 0.05 num=num+1    #если p < 0.05 то считаем исследование удачным +1
